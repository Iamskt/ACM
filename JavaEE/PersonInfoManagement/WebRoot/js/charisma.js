$(document).ready(function() {
	...
	var current_theme = $.cookie('current_theme') == null ? 'cerulean' : $
			.cookie('current_theme');
	switch_theme(current_theme);

	$('#themes a[data-value="' + current_theme + '"]').find('i')
			.addClass('icon-ok');
	...
	$('#is-ajax')
			.prop('checked', $.cookie('is-ajax') === 'true' ? true : false);
...
function docReady() {
	...
	$("input:checkbox, input:radio, input:file")
			.not('[data-no-uniform="true"],#uniform-is-ajax').uniform();
	...
	// makes elements soratble, elements that sort need to have id attribute to
	// save the result
	$('.sortable').sortable({
				revert : true,
				cancel : '.btn,.box-content,.nav-header',
				update : function(event, ui) {
					// line below gives the ids of elements, you can make ajax
					// call here to save it to the database
	...
	var elf = $('.file-manager').elfinder({
				url : 'misc/elfinder-connector/connector.php' // connector URL
																// (REQUIRED)
			}).elfinder('instance');
	...
	$('ul.gallery li').hover(function() {
		...
		$(this)
				.append('<div class="well gallery-controls">'
	...
	$('#toggle-fullscreen').button().click(function() {
		...
		if (!button.hasClass('active')) {
			...
			(document.webkitCancelFullScreen || document.mozCancelFullScreen || $.noop)
					.apply(document);
		}
	});
	...
	if ($('.tour').length && typeof(tour) == 'undefined') {
		...
		tour.addStep({
					element : ".span10:first", /*
												 * html element next to which
												 * the step popover should be
												 * shown
												 */
					placement : "top",
			...
			for (i = 0, iLen = an.length; i < iLen; i++) {
				...
				for (j = iStart; j <= iEnd; j++) {
					...
					$('<li ' + sClass + '><a href="#">' + j + '</a></li>')
							.insertBefore($('li:last', an[i])[0]).bind('click',
									function(e) {
										e.preventDefault();
