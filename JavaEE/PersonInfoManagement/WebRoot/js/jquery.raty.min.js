 ...
 * 
 * @version 2.4.5 @author Washington Botelho @documentation wbotelhos.com/raty
 * 
 */
	...
	var a = {
					...
					b("<img />", {
						src : d.opt.path
								+ ((!d.opt.score || d.opt.score < e)
										...
										: d.opt.starOn),
						alt : e,
						title : (e <= d.opt.hints.length && d.opt.hints[e - 1] !== null)
								? d.opt.hints[e - 1]
								: e
					}).appendTo(d);
					if (d.opt.space) {
		...
		bindAction : function() {
			...
			c.stars.bind(d, function(g) {
				...
				if (c.opt.half) {
					var f = parseFloat((g.pageX - b(this).offset().left)
							/ c.opt.size), i = (f > 0.5) ? 1 : 0.5;
				...
				c.score.val((c.opt.half || c.opt.precision)
						? e.data("score")
						: this.alt);
				if (c.opt.click) {
		...
		fixHint : function() {
			...
			if (!isNaN(e) && e > 0) {
				d = (e <= this.opt.hints.length && this.opt.hints[e - 1] !== null)
						? this.opt.hints[e - 1]
						: e;
			}
