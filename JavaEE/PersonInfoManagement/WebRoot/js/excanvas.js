...
if (!document.createElement('canvas').getContext) {
	...
	(function() {
		...
		contextPrototype.drawImage = function(image, var_args) {
			...
			if (this.m_[0][0] != 1 || this.m_[0][1] || this.m_[1][1] != 1
					|| this.m_[1][0]) {
				var filter = [];
			...
			vmlStr.push(' ">', '<g_vml_:image src="', image.src, '"',
					' style="width:', Z * dw, 'px;', ' height:', Z * dh, 'px"',
		...
		contextPrototype.stroke = function(aFill) {
			var W = 10;
			var H = 10;
			// Divide the shape into chunks if it's too long because IE has a
			// limit
			// somewhere for how long a VML shape can be. This simple division
			// does
			...
			for (var j = 0; j < this.currentPath_.length; j += chunkSize) {
				...
				for (var i = j; i < Math.min(j + chunkSize,
						this.currentPath_.length); i++) {
					if (i % chunkSize == 0 && i > 0) { // move into position
														// for next chunk
					...
					switch (p.type) {
						...
						case 'at' :
						case 'wa' :
							lineStr.push(' ', p.type, ' ', mr(p.x
											- this.arcScaleX_ * p.radius), ',',
		...
		function appendStroke(ctx, lineStr) {
			...
			lineStr.push('<g_vml_:stroke', ' opacity="', opacity, '"',
					' joinstyle="', ctx.lineJoin, '"', ' miterlimit="',
					ctx.miterLimit, '"', ' endcap="',
					processLineCap(ctx.lineCap), '"', ' weight="', lineWidth,
					'px"', ' color="', color, '" />');
		}
		...
		function appendFill(ctx, lineStr, min, max) {
			...
			if (fillStyle instanceof CanvasGradient_) {
				...
				if (width && height) {
					var deltaLeft = -min.x;
					var deltaTop = -min.y;
					lineStr.push('<g_vml_:fill', ' position="', deltaLeft
									/ width * arcScaleX * arcScaleX, ',',
		...
		contextPrototype.drawText_ = function(text, x, y, maxWidth, stroke) {
			...
			switch (textAlign) {
				...
				case 'right' :
					break;
				case 'end' :
					textAlign = elementStyle.direction == 'ltr'
							? 'right'
							: 'left';
					break;
				case 'start' :
					textAlign = elementStyle.direction == 'rtl'
							? 'right'
							: 'left';
					break;
				default :
			...
			// 1.75 is an arbitrary number, as there is no info about the text
			// baseline
			...
			lineStr.push('<g_vml_:line from="', -left, ' 0" to="', right,
					' 0.05" ', ' coordsize="100 100" coordorigin="0 0"',
			...
			lineStr.push('<g_vml_:skew on="t" matrix="', skewM, '" ',
					...
					encodeHtmlAttribute(text), '" style="v-text-align:',
					textAlign, ';font:', encodeHtmlAttribute(fontStyleString),
					'" /></g_vml_:line>');
